...
@Composable
fun SolicitacaoEquipamentoScreen(
    onMenuClick: ()-> Unit,
    viewModel: SolicitacaoEquipamentosViewModel = hiltViewModel()
) {
    ...
    Scaffold(
        ...
        topBar = { AppTopBar(title = "Solicitação - Equipamentos", onMenuClick = onMenuClick) }
    ) { padding ->
        ...
        if (uiState.isLoading) {
            ...
            LazyColumn(
                ...
                items(uiState.data) { solicitacao ->
                    SolicitacaoCard(solicitacao, viewModel)
                }
            }
    ...
    @Composable
    fun SolicitacaoCard(
        solicitacao: SolicitacaoEquipamentos,
        viewModel: SolicitacaoEquipamentosViewModel = hiltViewModel()
    ) {
        ...
        Card(
            ...
            Column(modifier = Modifier.padding(12.dp)) {
                ...
                Text(
                    "Almoxarife: ${solicitacao.nomeAlmoxarife}",
                    fontWeight = FontWeight.Medium
                )
                Text(
                    "Login Técnico: ${solicitacao.loginTecnico}",
                    fontWeight = FontWeight.Medium
                )
                Text(
                    "Quant. Equipamentos: ${solicitacao.equipamentos.size}",
                    fontWeight = FontWeight.Medium
                )
                ...
                Column(
                    ...
                    solicitacao.equipamentos.forEach { equip ->
                        Text(
                            "• ${equip.numeroSerie} - ${equip.tipo}",
                            fontSize = 14.sp,
                            modifier = Modifier.align(Alignment.CenterHorizontally)
                        )
                    }
                ...
                // Data formatada
                val dataFormatada = DateUtils.formatTimestamp(solicitacao.data)
                ...
                Row(
                    ...
                    Button(
                        onClick = {
                            viewModel.updateStatusSolicitacao(solicitacao, "Aceita")
                            Toast.makeText(
                                context,
                    ...
                    Button(
                        onClick = {
                            viewModel.updateStatusSolicitacao(solicitacao, "Rejeitada")
                            Toast.makeText(
                                context,
